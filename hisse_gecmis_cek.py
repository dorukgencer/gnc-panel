...
def main():
    ...
    print(f"{len(kodlar)} hisse icin ~{GUN_SAYISI} gunluk gecmis {GRUP_BOYUT}'li gruplarla cekiliyor...")
    ...
    for i in range(0, len(kodlar), GRUP_BOYUT):
        ...
        if df is None:
            print(f"  grup {i}-{i+len(parca)}: cekilemedi, atlaniyor (eski dosyalar korunur)")
            continue
        ...
        if not kod_k or not kap_k or not tar_k:
            print(f"  grup {i}-{i+len(parca)}: kolon bulunamadi, atlaniyor")
            continue
